...
function getConnectionId(connection) {
  if (typeof connection._stream === "object") {
    ...
    if (
      typeof connection._stream.rawUrl === "string" &&
      connection._stream.rawUrl.length > 0
    )
...
function getSubscriptionCount(id, connection) {
  ...
  if (!Counts.hasOwnProperty(connectionId))
    Counts[connectionId] = new Meteor.Collection("pagination-counts", {
      connection,
    });
...
class PaginationFactory {
  constructor(collection, settingsIn = {}) {
    ...
    this.connection =
      settingsIn && settingsIn.connection
        ? settingsIn.connection
        : Meteor.connection;
    ...
    if (!this.currentPage()) this.currentPage(settings.page);
    if (!this.perPage()) this.perPage(settings.perPage);
    if (!this.filters()) this.filters(settings.filters);
    if (!this.fields()) this.fields(settings.fields);
    if (!this.skip()) this.skip(settings.skip);
    if (!this.sort()) this.sort(settings.sort);
    if (!this.debug()) this.debug(settings.debug);
  ...
  _checkObservers() {
    ...
    if (this._activeObservers.hasOwnProperty(currentComputationId)) return;
    ...
    Tracker.onInvalidate((c) => {
      ...
      _.each(this._activeObservers, (value, id) => {
        if (!value) delete this._activeObservers[id];
      });
      ...
      if (c.stopped && this._activeObservers.hasOwnProperty(c._id)) {
        ...
        if (_.isEmpty(this._activeObservers)) {
          if (this.debug())
            console.log("Pagination", this.settings.get("name"), "unsubscribe");
  ...
  currentPage(page) {
    if (arguments.length === 1) {
      if (page >= 1) this.settings.set("page", page);
    } else return this.settings.get("page");
  }

  perPage(perPage) {
    if (arguments.length === 1) this.settings.set("perPage", perPage);
    else return this.settings.get("perPage");
  }

  filters(filters) {
    if (arguments.length === 1)
      this.settings.set("filters", !_.isEmpty(filters) ? filters : {});
    else return this.settings.get("filters");
  }

  fields(fields) {
    if (arguments.length === 1) this.settings.set("fields", fields);
    else return this.settings.get("fields");
  }

  skip(skip) {
    if (arguments.length === 1) this.settings.set("skip", skip);
    else return this.settings.get("skip");
  }

  sort(sort) {
    if (arguments.length === 1) this.settings.set("sort", sort);
    else return this.settings.get("sort");
  }
  ...
  debug(debug) {
    if (arguments.length === 1) this.settings.set("debug", debug);
    else return this.settings.get("debug");
  }
  ...
  getPage() {
    ...
    if (this.ready()) {
      const totalItems = getSubscriptionCount(
        `sub_${this.subscription.subscriptionId}`,
        this.connection,
      );
      this.settings.set("totalItems", totalItems);
      if (
        this.currentPage() > 1 &&
        totalItems <= this.perPage() * this.currentPage()
      )
